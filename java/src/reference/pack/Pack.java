...
//		}
	...
	private void place1stStrip() {
		...
		for (int i = 0; i < rotSteps; i++) {
			// rotate the geometry, align it to x axis (or y, depend on the value of preferX)
		...
		// the best rotation as well as the corresponding bounding box
		double[][] tp = M.rotate(trigos[rotid], first.outps);
		double[] bd = M.boundBox(tp);
		// rotate and move the frist one to the left bottom
		first.fix_rotate_move(trigos[rotid], new double[] { -bd[0], -bd[2] });
		
		// remove the object from the list1, add it to this pack
		movs.remove(sid);
		fixs.add(first);
	...
	public void packOneSheet(boolean Abey) {
		// pack first object
		place1stStrip();
		// pack the rest objects, from large one to small one
		int size = movs.size();
		...
		// update move list (not necessary?)
	...
	private boolean placeAnotherStrip_Abey(int sid) {
		...
		for (int i = 0; i < opl.length; i++) { // each vertex of new strip
			double[] p = opl[i]; // current pt
			// vector
			double[] d0 = M.sub(opl[(i - 1 + opl.length) % opl.length], p); // vector to previous vertex
			double[] d2 = M.sub(opl[(i + 1) % opl.length], p); // vector to next vertex
			// magnitude
			double mag0 = M.mag(d0);
			double mag2 = M.mag(d2);
			// normalized vector (cos and sin)
			double cb0 = d0[0] / mag0;
			double sb0 = d0[1] / mag0;
			double cb2 = d2[0] / mag2;
			double sb2 = d2[1] / mag2;

			for (Strip fixed : fixs) { // for each fixed strip
				...
				for (int j = 0; j < fopl.length; j++) { // each vertex of each fixed strip
					...
					for (int h = 0; h < 2; h++) { // two angles
						...
						// two way of rotation (what is this?)
						if (0 == h)
						...
						// translation, from fpol[j] to rot_opl[i]
						double[] trans = M.sub(v, rot_opl[i]); // *****************
						// translate the rotated geometry
						double[][] trans_rot_outpoly = M.move(trans, rot_opl);
						
						if (feasible(trans_rot_outpoly)) { // feasible placement
							// rotate and translate the inner geometry
							...
							// check bounding area, select the best one
	...
	/**
	 * non-overlap and inside the boundary?
	 * @param poly
	 * @return
	 */
